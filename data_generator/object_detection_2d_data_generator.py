...
class DataGenerator:
    ...
    def parse_wider_mat(self,
                        images_dirs,
                        annotations_files,
                        ret=False,
                        verbose=True):
        """
        This is a WIDERFace dataset mat file parser.
        ('class_id', 'xmin', 'ymin', 'xmax', 'ymax')

        Returns:
            None by default, optionally list for whichever are available of images, images filenames, labesl, image IDs,
            and a list indicating which boxes are annotated with the label "difficult".
        """
        ...
        if not annotations_files:
            self.labels = None
            annotations_files = [None] * len(images_dirs)

        for images_dir, annotations_file in zip(images_dirs, annotations_files):
            # Read the annotations files
            content = scipy.io.loadmat(annotations_file)
            event_list = content.get("event_list")
            file_list = content.get("file_list")
            bbx_list = content.get("face_bbx_list")
            self.dataset_size = 0
            # TODO: implement verbose

            if annotations_file is not None:
                # Parse mat file to get images
                for event_idx, event in enumerate(event_list):
                    folder = os.path.join(images_dir, event[0][0])
                    self.dataset_size += len(file_list[event_idx][0])
                    for im_idx, im in enumerate(file_list[event_idx][0]):
                        filename = "{}.jpg".format(im[0][0])
                        self.filenames.append(os.path.join(folder, filename))
                        face_bbx = bbx_list[event_idx][0][im_idx][0]
                        ...
                        for i in range(face_bbx.shape[0]):
                            ...
                            for item in self.labels_output_format:
                                box.append(item_dict[item])
                            boxes.append(box)
                            # No difficult/eval_neutral like VOC
                        self.labels.append(boxes)
        ...
        if ret:
            return self.images, self.filenames, self.lables, self.image_ids, self.eval_neutral
